...
class GenerateServiceCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Generate service class with store and update request classes';
    ...
    public function handle(): void
    {
        ...
        if (! $this->files->exists($this->targetPath)) {
            ...
            $this->info("✅ Service created: {$this->targetPath}");
            $created++;
        } else {
            $this->warn("⚠️  Service already exists: {$this->targetPath}");
        }

        if (! $this->files->exists($this->storeRequestPath)) {
            $this->files->put($this->storeRequestPath, $this->getTemplateStoreReqFileContent());
            $this->info("✅ Store Request created: {$this->storeRequestPath}");
            $created++;
        } else {
            $this->warn("⚠️  Store Request already exists: {$this->storeRequestPath}");
        }

        if (! $this->files->exists($this->updateRequestPath)) {
            $this->files->put($this->updateRequestPath, $this->getTemplateUpdateReqFileContent());
            $this->info("✅ Update Request created: {$this->updateRequestPath}");
            $created++;
        } else {
            $this->warn("⚠️  Update Request already exists: {$this->updateRequestPath}");
        }

        if ($created > 0) {
            $this->info("🎉 Successfully created {$created} file(s) for service: {$this->singularClassName}");
        } else {
